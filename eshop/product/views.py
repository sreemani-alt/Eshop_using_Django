...
def update_product(request,pk):
    ...
    if product.user != request.user:
        return Response({ 'error': 'You cannot update this product ' }, status=status.
        HTTP_403_FORBIDDEN)
...
def delete_product(request,pk):
    ...
    if product.user != request.user:
        return Response({ 'error': 'You cannot delete this product ' }, status=status.
        HTTP_403_FORBIDDEN)
...
def create_review(request, pk):
    ...
    if data['rating'] <= 0 or data['rating'] > 5:
        return Response({'error': 'Please select rating between 1-5' }, status=status.
...
def delete_review(request, pk):
    ...
    if review.exists():
        ...
        return Response({'error': 'Review not found' }, status=status.
        HTTP_404_NOT_FOUND)
